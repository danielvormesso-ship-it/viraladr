...
const PRESET_HASHTAGS: Record<string, { tags: string[]; group: string }> = {
  ...
  'frutinovela':      { tags: ['frutasia','frutas','novelafrutas','moranguete','abacatudo','bananildo','noveladefrutas','frutasIA','frutinovela','frutasanimadas'], group: 'novelas' },
  'mininovela':       { tags: ['mininovela','novelinha','novelatiktok','dramabr','novelabrasileira','micronovela','novelacurta','dramatiktok','dramabrasil','serietiktok'], group: 'novelas' },
  'cortesdenovela':   { tags: ['cortesdenovela','cortesdeserie','cortesdefilme','trechosdefilmes','trechosdeseries','cenasdenovela','cortesvirais','cortesbr','cenasiconicas','melhoresceanas'], group: 'novelas' },
  ...
  'decoração':        { tags: ['decoracao','decoracaodecasa','homedecor','casanova','transformacaodacasa','moveis','decoracaobr','decoracaosimples','cantinhodecorado','diydecoração'], group: 'casa' },
  'reforma':          { tags: ['reforma','reformadecasa','reformabr','antesedepoisreforma','reformabarata','movelplanejado','projetodecor','reformacasa','obrareforma','reformando'], group: 'casa' },
  'faxina':           { tags: ['faxina','faxinacompleta','limpezadacasa','casalimpa','limpezaprofunda','faxinadacasa','rotinadelimpeza','limpandoacasa','faxinabr','faxinarapida'], group: 'casa' },
  'diarista':         { tags: ['diarista','diaristabrasileira','diaristaprofissional','faxineira','limpezadomestica','diaristabr','limpeiminacasa','casacheirosa','limpandotudo','faxinapesada'], group: 'casa' },
...
const FOREIGN_ES_WORDS = /\b(pero|muy|esto|hola|gracias|hermano|bueno|jaja|amigo|novia|pareja|siempre|cuando|donde|también|tambien|porque|aunque|todavía|todavia|necesito|puedo|quiero|tiene|puede|vamos|mejor|peor|nunca|otra|otro|mismo|aquí|ahora|entonces|después|antes|todos|nada|algo|alguien|nadie|mucho|poco|demasiado|bastante|cada|algún|ningún|cualquier|bromas|broma|loquendo|dimision|dimisión|gobierno|presidente|elecciones|espana|españa|companero|compañero|chicos|chicas|mira esto|increíble|increible|verdad|mentira|cuidado|peligro|tonto|tonta|guapo|guapa|novio|chistoso|chistosa|gracioso|graciosa|jajaja|miren|playa)\b/gi;
const FOREIGN_FLAG_PATTERN = /🇦🇴|🇵🇹|🇪🇸|🇦🇷|🇨🇴|🇲🇽|🇨🇱|🇵🇪|🇻🇪|🇪🇨|🇺🇾/g;
const FOREIGN_FR_WORDS = /\b(c'est|avec|pour|dans|nous|vous|leur|quand|chez|sont|mais|tout|très|même|être|faire|comme|peut|donc|alors|cette|aussi|encore|entre|après|avant|rien|toujours|jamais|quelque|chaque|depuis|pendant|sans|vers|ici|ailleurs|bonjour|merci|oui|salut|putain|merde|trop|voilà|quoi|bah|ouais|nan|chéri|chérie|les|jeudis|théâtre|theatre|sensation|échecs|checs|magnifique|formidable|incroyable)\b/gi;
const FOREIGN_IT_WORDS = /\b(questa|quello|perché|anche|dove|cosa|ogni|tutto|niente|qualcosa|qualcuno|nessuno|troppo|abbastanza|già|adesso|dopo|insieme|senza|contro|circa|pensi|sono|voglio|posso|bene|male|grazie|ciao|buongiorno|allora|molto|bello|ragazza|ragazzi|andiamo|stai|faccio|vuoi|sai|vieni|aspetta|scattano|macchina|bellissimo|bellissima|buonasera|perfetto|perfetta|mangiare|piace|prego)\b/gi;
