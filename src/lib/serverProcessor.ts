...
export async function pollJobStatus(
  ...
  const MAX_STALLED_PROGRESS_MS = 60 * 1000; // 60s sem avanço real — cancela job travado
  ...
  while (Date.now() - pollStartedAt < MAX_POLL_DURATION_MS) {
    ...
    if (progressKey !== lastProgressKey) {
      ...
      throw new Error('Job sem progresso por mais de 60 segundos');
    }
    ...
    // C3: Aggressive polling — 500ms during active work, 1s while queued
    const adaptiveInterval = data.status === 'queued' ? 1000 : 500;
